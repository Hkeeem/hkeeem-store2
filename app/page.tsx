...
useEffect(()=>{ try{ const u=localStorage.getItem('hkeeem_user'); if(u) setUser(JSON.parse(u)); const l=localStorage.getItem('hkeeem_user_location'); if(l) setUserLoc(JSON.parse(l)); const c=localStorage.getItem('hkeeem_city'); if(c) setCityManual(JSON.parse(c)); const f=localStorage.getItem('hkeeem_favs'); if(f) setFavs(JSON.parse(f)) }catch{} },[])
...
<img src="/Hkeeem_logo_en-2.png" alt="Hkeeem" className="w-11 h-11 rounded-xl border border-[#EADFC9] bg-white object-contain p-1 shadow-sm"/>
<span className="font-black text-lg">عروض<span style={{color:GOLD}}>كم</span></span>
{cityManual&&<span className="hidden md:inline-flex px-3 py-1 rounded-full text-xs font-bold bg-[#FFF8E6] border border-[#EADFC9] text-[#7A5A16]">📍 {cityManual.label}</span>}
</div>
<div className="flex items-center gap-2"><button onClick={()=>setAuthOpen(true)} className="px-4 h-10 rounded-full bg-white border border-[#EADFC9] text-xs font-black">{user?`👋 ${user.name}`:'👤 دخول'}</button><button onClick={fetchLocation} className="px-4 h-10 rounded-full text-white text-xs font-black shadow" style={{background:`linear-gradient(135deg, ${GOLD_DARK}, ${GOLD_LIGHT})`}}>{locLoading?'جاري...':'📍 موقعي'}</button></div>
</div>
</header>
<main className="max-w-6xl mx-auto px-4">
<div className="mt-4 relative"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="ابحث: أرخص مكيف، عطور، بنده..." className="w-full h-12 pr-11 pl-4 rounded-2xl border-2 border-[#EADFC9] bg-white outline-none text-sm focus:border-[#B68A2E]"/><span className="absolute right-4 top-1/2 -translate-y-1/2">🔍</span></div>
...
<div className="relative"><img src={o.image} alt={o.title} className="h-48 w-full object-cover"/><span className="absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-black text-white" style={{background:`linear-gradient(135deg, ${GOLD_DARK}, ${GOLD})`}}>-{o.discount}%</span><button onClick={()=>toggleFav(o.id)} className="absolute top-3 left-3 w-9 h-9 rounded-full bg-white/95 border grid place-items-center">{isFav?'❤️':'🤍'}</button>{o.distance!==null&&<span className="absolute bottom-3 left-3 px-2.5 py-1 rounded-full bg-black/80 text-white text-xs font-bold">📍 {formatDistance(o.distance)}</span>}</div>
<div className="p-4 flex flex-col flex-1"><h3 className="font-bold text-base line-clamp-2 min-h-12">{o.title}</h3><div className="mt-3 flex items-baseline gap-2"><span className="font-black text-lg" style={{color:GOLD_DARK}}>{formatPrice(o.price)}</span><span className="text-sm line-through text-zinc-400">{formatPrice(o.old_price)}</span></div><div className="mt-2 text-xs font-bold"><span className="px-2.5 py-1 rounded-full bg-red-50 border border-red-200 text-red-600">⏳ {countdown}</span></div><button onClick={()=>window.open(`https://www.google.com/maps/dir/?api=1&destination=${o.location.lat},${o.location.lng}`,'_blank')} className="mt-4 h-11 rounded-full text-white font-black" style={{background:GOLD_DARK}}>الوصول 🚗</button></div>
</article>
)})}
</section>
</main>
