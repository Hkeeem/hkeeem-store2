...
const LIGHT_GRAY_BG = "#F2F3F5"
const CARD_BG = "#F9FAFB"
const CARD_SOFT = "#EDEEF0"
...
const HERO = [
 ...
 {store:'بنده', d:55, t:'سلة التوفير الكبرى', p:89, old:199, img:'https://images.unsplash.com/photo-1542838132-92c53300491e?w=900', g:'from-zinc-700 to-zinc-900'},
...
const OFFERS: Offer[] = [
 {id:1,title:'عطر حكيم الملكي 100 مل',store:'متجر حكيم',category:'عطور',price:199,old_price:349,discount:43,image:'https://images.unsplash.com/photo-1594035910387-fea47794261f?w=600',isOwn:true,lat:21.54,lng:39.17,city:'جدة'},
 {id:2,title:'محفظة جلد مع ساعة',store:'متجر حكيم',category:'ملابس',price:399,old_price:619,discount:35,image:'https://images.unsplash.com/photo-1523170335258-f5ed11844a49?w=600',isOwn:true,lat:21.54,lng:39.17,city:'جدة'},
...
export default function Page(){
 ...
 const openSaudi = ()=>{ const sorted=[...BEST_SAUDI].sort((a,b)=>b.strength-a.strength); setOpenStores('saudi'); const txt=`عروض المتاجر الاقوى بالترتيب. الاول ${sorted[0].name} بقوة ${sorted[0].strength}. الثاني ${sorted[1].name}. الثالث ${sorted[2].name}`; setTimeout(()=>speak(txt),300); }
 const openElec = ()=>{ const sorted=[...BEST_ELEC].sort((a,b)=>b.strength-a.strength); setOpenStores('elec'); const txt=`عروض المتاجر الالكترونية الاقوى. الاول ${sorted[0].name} بقوة ${sorted[0].strength}`; setTimeout(()=>speak(txt),300); }

 return(<div dir="rtl" style={{background: dark? '#09090b' : LIGHT_GRAY_BG}} className={`min-h-screen leading-7 ${dark?'text-zinc-100':'text-zinc-900'}`}>
  <header className="sticky top-0 z-30 backdrop-blur border-b" style={{background: dark? '#18181b' : CARD_BG, borderColor: BORDER_GRAY}}><div className="max-w-7xl mx-auto px-4 h-14 flex justify-between items-center"><div className="font-black text-xl">عروض<span style={{color:VIOLET}}>كم</span></div><div className="flex gap-2 items-center"><button onClick={()=>setDark(!dark)} className="w-10 h-10 rounded-full border bg-white grid place-items-center">{dark?'نهار':'ليل'}</button><div className="px-4 h-10 rounded-full text-white grid place-items-center font-bold text-sm" style={{background:`linear-gradient(to right, ${VIOLET}, #A21CAF)`}}>السلة {cart}</div></div></div></header>

  <main className="max-w-7xl mx-auto px-3 pb-24">
   <div className="mt-4 relative h-[300px] rounded- overflow-hidden bg-black text-white shadow-xl">{HERO.map((s,i)=><div key={i} className={`absolute inset-0 transition-opacity duration-700 ${i===a?'opacity-100':'opacity-0'}`}><div className={`absolute inset-0 bg-gradient-to-r ${s.g}`}/><div className="relative h-full flex flex-col md:flex-row p-6 gap-4 items-center"><div className="flex-1 space-y-2"><span className="px-3 py-1 rounded-full bg-white text-black text-xs font-bold">{s.store}</span><div className="text-2xl font-black">{s.t}</div><div className="flex gap-2 items-baseline"><span className="text-xl font-black">{s.p} ر.س</span><span className="line-through opacity-60 text-sm">{s.old}</span></div><button onClick={()=>setCart(c=>c+1)} className="px-6 h-10 rounded-full bg-white text-black font-bold text-sm">احصل الان</button></div><img src={s.img} className="w-[90%] md:w-[36%] h-36 md:h-[70%] object-cover rounded-"/></div></div>)}</div>

   <div className="mt-6 grid md:grid-cols-3 gap-4">
    <div className="rounded- border p-5" style={{background: CARD_BG, borderColor: BORDER_GRAY}}><h3 className="font-black">التصنيفات</h3><div className="text-xs opacity-60 mt-1">اختر قسم لتصفية العروض - صفين</div><div className="mt-4 grid grid-cols-4 gap-2.5">{CATS_BOX.map(c=><button key={c.n} onClick={()=>setCat(c.n)} className="h-16 rounded-2xl border font-bold text-xs text-center p-1" style={{background: cat===c.n?VIOLET:CARD_SOFT, color: cat===c.n?'white':'black', borderColor: cat===c.n?VIOLET:BORDER_GRAY}}>{c.n}</button>)}</div></div>

    <button onClick={openSaudi} className="rounded- border p-5 text-right w-full hover:shadow-md transition" style={{background: CARD_BG, borderColor: BORDER_GRAY}}><div className="flex justify-between items-center"><h3 className="font-black">عروض المتاجر</h3><span className="text-xs px-2 py-1 rounded-full text-white" style={{background:VIOLET}}>اضغط للفتح ويتحدث</span></div><div className="text-xs opacity-60 mt-1">اذا لمست الملف يفتح ويظهر المتاجر الاقوى بالترتيب ويتحدث</div><div className="mt-4 grid grid-cols-3 gap-2.5">{BEST_SAUDI.map(s=><div key={s.name} className="rounded-2xl border p-2.5 text-center" style={{background: s.name==='متجر حكيم'?'#EDE9FE':CARD_SOFT, borderColor: BORDER_GRAY}}><div className="font-bold text-xs">{s.name} {s.tag==='مفعل'&&<span className="text-[9px] px-1 py-0.5 rounded-full text-white" style={{background:VIOLET}}>مفعل</span>}</div><div className="text- opacity-60 mt-0.5">{s.rating} - {s.offers} عرض</div></div>)}</div></button>

    <button onClick={openElec} className="rounded- border p-5 text-right w-full hover:shadow-md transition" style={{background: CARD_BG, borderColor: BORDER_GRAY}}><div className="flex justify-between items-center"><h3 className="font-black">عروض المتاجر الالكترونية</h3><span className="text-xs px-2 py-1 rounded-full bg-zinc-900 text-white">يتحدث</span></div><div className="text-xs opacity-60 mt-1">ترتيب حسب قوة العروض</div><div className="mt-4 grid grid-cols-2 gap-2.5">{BEST_ELEC.map(s=><div key={s.name} className="rounded-2xl border p-3" style={{background:CARD_SOFT, borderColor:BORDER_GRAY}}><div className="font-bold text-xs">{s.name}</div><div className="text- opacity-60">قوة {s.strength}</div></div>)}</div></button>
   </div>

   <div className="mt-6 rounded- border p-5 flex flex-col md:flex-row justify-between items-center gap-3" style={{background: CARD_BG, borderColor: BORDER_GRAY}}><div><div className="font-black">المكتب العقاري - محسن الحكمي - وسيط معتمد</div><div className="text-xs opacity-70 mt-1">جدة - توثيق عقود - تسويق احترافي</div></div><div className="flex gap-2"><a href="https://dealapp.sa/ar/profile/67c08063ca5bafdb59e3d8d4" target="_blank" className="px-5 h-10 rounded-full text-white text-sm font-bold grid place-items-center" style={{background:VIOLET}}>فتح الملف العقاري</a><button onClick={()=>speak('المكتب العقاري محسن الحكمي وسيط معتمد في جدة')} className="px-4 h-10 rounded-full border bg-white text-xs font-bold">استماع</button></div></div>

   <div className="mt-6 grid grid-cols-2 gap-4">{filtered.map((o:any)=><div key={o.id} className="rounded-2xl border overflow-hidden" style={{background: CARD_BG, borderColor: BORDER_GRAY}}><div className="relative"><img src={o.image} className="h-40 w-full object-cover"/><span className="absolute top-3 right-3 px-2.5 py-1 rounded-full text-white text-xs font-bold" style={{background:VIOLET}}>-{o.discount}%</span></div><div className="p-3"><div className="text-xs opacity-60">{o.store}</div><div className="font-bold text-sm">{o.title}</div><div className="flex justify-between items-center mt-2"><span className="font-black" style={{color:VIOLET}}>{o.price} ر.س</span><button onClick={()=>setCart(c=>c+1)} className="w-8 h-8 rounded-full bg-zinc-900 text-white">+</button></div></div></div>)}</div>
  </main>

  {openStores&&<div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm grid place-items-center p-4" onClick={()=>{setOpenStores(null); window.speechSynthesis.cancel();}}><div onClick={e=>e.stopPropagation()} className="w-full max-w-[420px] rounded- p-5" style={{background: CARD_BG, border:`1px solid ${BORDER_GRAY}`}}><div className="flex justify-between items-center"><h3 className="font-black">{openStores==='saudi'?'عروض المتاجر - الاقوى بالترتيب':'عروض المتاجر الالكترونية'}</h3><button onClick={()=>{setOpenStores(null); window.speechSynthesis.cancel();}} className="w-8 h-8 rounded-full bg-zinc-100 grid place-items-center">اغلاق</button></div><div className="mt-3 flex gap-2"><button onClick={()=>{const sorted=openStores==='saudi'?[...BEST_SAUDI].sort((a,b)=>b.strength-a.strength):[...BEST_ELEC].sort((a,b)=>b.strength-a.strength); const t=sorted.map((s,i)=>`الترتيب ${i+1} ${s.name} بقوة ${s.strength}`).join('. '); speak(t)}} className="px-3 h-9 rounded-full text-white text-xs font-bold" style={{background:VIOLET}}>{speaking?'يتحدث الان':'اعادة التحدث'}</button></div><div className="mt-4 space-y-2.5">{(openStores==='saudi'?[...BEST_SAUDI].sort((a,b)=>b.strength-a.strength):[...BEST_ELEC].sort((a,b)=>b.strength-a.strength)).map((s,i)=><div key={s.name} className="rounded-2xl border p-4 flex justify-between items-center" style={{background: i===0?'#EDE9FE':CARD_SOFT, borderColor: BORDER_GRAY}}><div className="flex gap-3 items-center"><span className="w-8 h-8 rounded-full grid place-items-center font-black text-xs" style={{background: i===0?VIOLET:i===1?'#27272a':'#e4e4e7', color: i<=1?'white':'black'}}>{i+1}</span><div><div className="font-bold text-sm">{s.name}</div><div className="text-xs opacity-60">{s.rating} - {s.offers} عرض</div></div></div><div className="font-black text-sm" style={{color:VIOLET}}>قوة {s.strength}</div></div>)}</div></div></div>}
