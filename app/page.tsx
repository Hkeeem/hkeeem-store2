...
export default function Page(){
 ...
 useEffect(()=>{
   ...
   if(savedCart &&!isNaN(Number(savedCart))) setCart(Number(savedCart))
 },[])

 useEffect(()=>{
   if(openStores) return
   const id = window.setInterval(()=> setActive(x=> (x+1)%HERO.length), 4000)
 ...
 useEffect(()=>{
   const onKey = (e:KeyboardEvent)=>{ if(e.key==='Escape'){ setOpenStores(null); window.speechSynthesis?.cancel(); setSpeaking(false) } }
 ...
 const openList = useCallback((type:'saudi'|'elec')=>{
   ...
   setTimeout(()=> speak(`${intro}، ${sentence}`), 200)
 },[speak])

 return(
 <div dir="rtl" suppressHydrationWarning style={{background:T.bg, color:T.text}} className="min-h-screen transition-colors">
  <header className="sticky top-0 z-30 backdrop-blur border-b" style={{background: `${T.card}ee`, borderColor:T.border}}>
   <div className="max-w-7xl mx-auto px-4 h-14 flex justify-between items-center">
    <div className="font-black text-xl tracking-tight">عروض<span style={{color:VIOLET}}>كم</span></div>
    <div className="flex gap-2">
     <button aria-label="تبديل المظهر" onClick={()=>setDark(v=>!v)} className="w-10 h-10 rounded-full border grid place-items-center text-xs font-bold" style={{background:T.soft, borderColor:T.border}}>{dark?'نهار':'ليل'}</button>
  ...
  <main className="max-w-7xl mx-auto px-3 pb-24">
   <div className="mt-4 relative h-[280px] rounded-3xl overflow-hidden bg-black text-white">
    ...
    <div className="absolute bottom-3 right-1/2 translate-x-1/2 flex gap-1.5">{HERO.map((_,i)=><span key={i} className={`w-2 h-2 rounded-full transition-all ${i===active?'w-6 bg-white':'bg-white/40'}`}/>)}</div>
   </div>
   ...
   <div className="mt-6 grid md:grid-cols-3 gap-4">
    <div className="rounded-3xl border p-5" style={{background:T.card, borderColor:T.border}}>
     ...
     <div className="mt-4 grid grid-cols-4 gap-2.5">{CATS_BOX.map(c=><button key={c.n} onClick={()=>setCat(c.n)} className="h-16 rounded-2xl border font-bold text-xs transition-all active:scale-95" style={{background: cat===c.n?VIOLET:T.soft, color: cat===c.n?'white':T.text, borderColor: cat===c.n?VIOLET:T.border}}>{c.n}</button>)}</div>
    </div>

    <button onClick={()=>openList('saudi')} className="rounded-3xl border p-5 text-right w-full hover:shadow-md transition text-right" style={{background:T.card, borderColor:T.border}}>
     ...
     <div className="mt-4 grid grid-cols-3 gap-2.5">{BEST_SAUDI.slice(0,6).map(s=><div key={s.name} className="rounded-2xl border p-2.5 text-center pointer-events-none" style={{background: s.name==='متجر حكيم'? (dark?'#2e1065':'#F5F0FF'):T.soft, borderColor:dark?'#4c1d95':'#E9D5FF'}}><div className="font-bold text-xs truncate">{s.name}</div><div className="text-[11px] opacity-60 mt-0.5">{s.rating} · {s.offers} عرض</div></div>)}</div>
    </button>

    <button onClick={()=>openList('elec')} className="rounded-3xl border p-5 text-right w-full hover:shadow-md transition" style={{background:T.card, borderColor:T.border}}>
     ...
     <div className="mt-4 grid grid-cols-2 gap-2.5">{BEST_ELEC.map(s=><div key={s.name} className="rounded-2xl border p-3 pointer-events-none" style={{background:T.soft, borderColor:T.border}}><div className="font-bold text-xs">{s.name}</div><div className="text-[11px] opacity-60">قوة {s.strength}</div></div>)}</div>
    </button>
   </div>
   ...
   <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
    {filtered.length===0? <div className="col-span-full text-center py-10 opacity-60 text-sm">لا توجد عروض في {cat} حاليا</div> :
     filtered.map(o=><div key={o.id} className="rounded-2xl border overflow-hidden hover:shadow-sm transition" style={{background:T.card, borderColor:T.border}}><img src={o.image} alt={o.title} loading="lazy" className="h-40 w-full object-cover"/><div className="p-3"><div className="text-xs opacity-60">{o.store} · {o.city}</div><div className="font-bold text-sm mt-1 leading-snug line-clamp-2">{o.title}</div><div className="flex justify-between items-center mt-3"><span className="font-black" style={{color:VIOLET}}>{o.price} ر.س</span><button onClick={()=>{setCart(c=>c+1); show('أضيف للسلة')}} className="w-8 h-8 rounded-full bg-zinc-900 text-white grid place-items-center active:scale-90 transition">+</button></div></div></div>)}
   </div>
  </main>
  ...
  {openStores && <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm grid place-items-center p-4" onClick={()=>{setOpenStores(null); stopSpeak()}}>
    <div role="dialog" aria-modal="true" onClick={e=>e.stopPropagation()} className="w-full max-w-[420px] rounded-3xl p-5 max-h-[80vh] overflow-auto shadow-2xl" style={{background:T.card, border:`1px solid ${T.border}`}}>
     <div className="flex justify-between items-center"><h3 className="font-black">{openStores==='saudi'?'المتاجر الأقوى بالترتيب':'المتاجر الإلكترونية'}</h3><button onClick={()=>{setOpenStores(null); stopSpeak()}} className="w-8 h-8 rounded-full grid place-items-center" style={{background:T.soft}}>✕</button></div>
     <button onClick={()=>{ const list=openStores==='saudi'?BEST_SAUDI:BEST_ELEC; const sorted=[...list].sort((a,b)=>b.strength-a.strength); speak(sorted.map((s,i)=>`الترتيب ${i+1} ${s.name} بقوة ${s.strength}`).join('. '))}} className="mt-3 px-4 h-9 rounded-full text-white text-xs font-bold" style={{background:VIOLET}}>{speaking?'يتحدث الآن…':'إعادة التحدث'}</button>
     <div className="mt-4 space-y-2.5">{(openStores==='saudi'?BEST_SAUDI:BEST_ELEC).slice().sort((a,b)=>b.strength-a.strength).map((s,i)=><div key={s.name} className="rounded-2xl border p-4 flex justify-between items-center" style={{background: i===0?(dark?'#2e1065':'#F5F0FF'):T.soft, borderColor: i===0?'#8b5cf6':T.border}}><div className="flex gap-3 items-center"><span className="w-8 h-8 rounded-full grid place-items-center font-black text-xs" style={{background: i===0?VIOLET:T.border, color:i===0?'white':T.text}}>{i+1}</span><div><div className="font-bold text-sm">{s.name}</div><div className="text-xs opacity-60">{s.rating} · {s.offers} عرض</div></div></div><div className="font-black text-sm" style={{color:VIOLET}}>{s.strength}</div></div>)}</div>
    </div>
  </div>}

  {toast && <div aria-live="polite" className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-zinc-900 text-white px-5 py-2.5 rounded-full text-sm z-50 shadow-lg animate-in fade-in slide-in-from-bottom-2">{toast}</div>}
 </div>)
     }
