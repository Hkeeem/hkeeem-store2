...
const PROFILE_MSG = "يسعدني استقبال جميع طلباتكم وعروضكم العقارية، وسأحرص على تقديم خدمة احترافية وسريعة حتى الوصول إلى أفضل فرصة تناسب احتياجكم"
...
const CATS = [
  ...
  { n:'المساعد الاقتصادي', i:'🤖' }, { n:'إلكترونيات', i:'📱' },
]

function OfferCard({ o, onGo }: { o:Offer, onGo:(o:Offer)=>void }){
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-[20px] border border-zinc-100 dark:border-zinc-800 overflow-hidden flex flex-col shadow-sm">
      ...
      <div className="p-3 flex-1 flex flex-col">
        ...
        <button onClick={()=>onGo(o)} className="mt-3 w-full h-9 rounded-full bg-[#6D28D9] text-white text-[13px] font-black">اذهب للعرض →</button>
      </div>
    </div>
  )
}
...
export default function Page(){
  ...
  const [aiMessages,setAiMessages]=useState([{role:'assistant',text:'أهلا! أنا مساعد حكيم 💜 اسألني عن أي عرض'}] as any)
  ...
  const runAI = (q:string)=>{ if(!q.trim()) return; const l=q.toLowerCase(); let res:Offer[]=OFFERS.filter(x=>x.store==='متجر حكيم').slice(0,3); if(l.includes('ساعة')) res=OFFERS.filter(o=>o.title.includes('ساعة')); if(l.includes('200')) res=OFFERS.filter(o=>o.price<=250); setAiMessages((p:any)=>[...p,{role:'user',text:q},{role:'assistant',text:`لقيت ${res.length} عروض`} ]); setAiResults(res); setAiInput('') }
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#FFFBFF] text-zinc-900 pb-[88px]">
      <header className="sticky top-0 z-20 bg-white/90 backdrop-blur border-b border-violet-100">
        <div className="max-w-6xl mx-auto px-4 h-[60px] flex items-center justify-between"><h1 className="font-black text-[18px]">عروض<span className="text-[#6D28D9]">كم</span></h1><button onClick={()=>setShowCart(true)} className="h-9 px-4 rounded-full bg-[#6D28D9] text-white text-sm font-bold">🛒 {cart.length>0?cart.reduce((s,i)=>s+i.qty,0):''}</button></div>
      </header>

      <div className="max-w-6xl mx-auto px-4 pt-4">
        <div className="flex gap-2 overflow-auto pb-2">{CATS.map(c=><button key={c.n} onClick={()=>setCat(c.n)} className={`whitespace-nowrap h-9 px-4 rounded-full border text-[13px] font-bold ${cat===c.n?'bg-[#6D28D9] text-white border-[#6D28D9]':'bg-white border-zinc-200'}`}>{c.i} {c.n}</button>)}</div>
        ...
        {cat==='المساعد الاقتصادي'?(
          ...
          <div className="mt-6 space-y-7">
            ...
            <section><div className="flex justify-between items-center mb-3"><h2 className="font-black">📉 نزل سعره اليوم</h2><span className="text-[11px] bg-green-100 text-green-700 px-2 py-1 rounded-full font-bold">توفير حتى 56%</span></div><div className="grid grid-cols-2 md:grid-cols-4 gap-3">{priceDropped.map(o=><OfferCard key={o.id} o={o} onGo={goOffer} />)}</div></section>
            ...
            {/* بطاقة مكتبك العقاري - جديدة */}
            <section className="mt-8 rounded-[24px] bg-gradient-to-br from-zinc-900 via-zinc-800 to-[#1a1625] text-white p-6 border border-white/10 shadow-xl overflow-hidden relative">
              <div className="absolute top-0 left-0 w-40 h-40 bg-[#6D28D9]/30 blur-[60px] rounded-full" />
              <div className="relative">
                <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur px-3 h-7 rounded-full text-[11px] font-bold border border-white/10">🏡 مكتب محسن الحكمي العقاري • موثوق</div>
                <h3 className="mt-3 font-black text-[18px] leading-7">يسعدني استقبال جميع طلباتكم وعروضكم العقارية</h3>
                <p className="mt-2 text-[13px] leading-6 text-white/70">سأحرص على تقديم خدمة احترافية وسريعة حتى الوصول إلى أفضل فرصة تناسب احتياجكم</p>
                <div className="mt-4 flex flex-wrap gap-2">
                  <a href={PROFILE_LINK} target="_blank" className="h-11 px-5 rounded-full bg-white text-zinc-900 font-black text-[13px] grid place-items-center hover:bg-zinc-100 transition">زيارة ملفي العقاري ↗</a>
                  <button onClick={()=>setShowContact(true)} className="h-11 px-5 rounded-full bg-[#6D28D9] text-white font-black text-[13px]">تواصل واتساب</button>
                </div>
                <div className="mt-4 flex items-center gap-2 text-[11px] text-white/50"><span>🔗</span><span className="truncate">{PROFILE_LINK}</span></div>
              </div>
            </section>
          </div>
        )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-[72px] bg-white/95 backdrop-blur-xl border-t flex justify-around items-center px-2 z-30">
        ...
        <button onClick={()=>setShowContact(true)} className="flex flex-col items-center gap-1 text-zinc-400"><span className="text-[20px]">👤</span><span className="text-[10px] font-bold">الحساب</span></button>
      </nav>

      {showContact&&<div className="fixed inset-0 z-50 bg-black/60 grid place-items-center p-4" onClick={()=>setShowContact(false)}><div className="w-full max-w-[360px] bg-white rounded-[24px] p-6" onClick={e=>e.stopPropagation()}><h3 className="font-black text-[16px]">تواصل مع محسن الحكمي</h3><p className="mt-3 text-[13px] leading-6 text-zinc-600">{PROFILE_MSG}</p><a href={PROFILE_LINK} target="_blank" className="mt-4 h-11 w-full rounded-full bg-zinc-900 text-white font-bold grid place-items-center text-sm">فتح الملف العقاري على DealApp</a><a href={`https://wa.me/?text=${encodeURIComponent(PROFILE_MSG+" \n"+PROFILE_LINK)}`} target="_blank" className="mt-2 h-11 w-full rounded-full bg-[#25D366] text-white font-bold grid place-items-center text-sm">مشاركة عبر واتساب</a><button onClick={()=>setShowContact(false)} className="mt-2 w-full h-10 rounded-full border text-sm font-bold">إغلاق</button></div></div>}
      ...
      <footer className="mt-10 border-t bg-white py-6 text-center"><div className="text-[12px] font-bold">مكتب محسن الحكمي العقاري</div><div className="mt-1 text-[11px] opacity-60 max-w-[32ch] mx-auto leading-5">{PROFILE_MSG}</div><a href={PROFILE_LINK} className="mt-2 inline-block text-[11px] text-[#6D28D9] font-bold underline">dealapp.sa/ar/profile/67c08063ca5bafdb59e3d8d4</a><div className="mt-3 text-[10px] opacity-40">© 2026 عروضكم - متجر حكيم</div></footer>
    </div>
  )
}
