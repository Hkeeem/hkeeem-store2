...
export default function Page(){
  ...
  const filteredOffers = useMemo(()=>{ if(!nearbyOnly||!refPoint) return sortedOffers; return sortedOffers.filter(o=>o.distance!==null&&o.distance<=radiusKm) },[sortedOffers,nearbyOnly,radiusKm,refPoint])
    ...
    <div className={`min-h-screen pb-28 ${BEIGE_BG} text-[#1F1B16]`} dir="rtl">
      <header className="sticky top-0 z-30 backdrop-blur-xl bg-[#FFFCF6]/90 border-b border-[#EADFC9]">
        <div className="max-w-7xl mx-auto px-4 h-16 flex justify-between items-center">
          <div className="flex items-center gap-2.5"><div className="w-9 h-9 rounded-xl border border-[#EADFC9] bg-white grid place-items-center font-black" style={{color:GOLD}}>ح</div><span className="font-black text-lg">عروض<span style={{color:GOLD}}>كم</span></span>{currentLabel&&<span className="hidden md:inline-flex px-3 py-1 rounded-full text-xs font-bold border bg-[#FFF8E6] text-[#7A5A16] border-[#EADFC9]">{currentLabel}</span>}</div>
      ...
      <main className="max-w-7xl mx-auto px-4">
        {tab==='home'&&<>
          ...
          <div className="mt-8 mb-5"><h2 className="font-black text-base">كل العروض مرتبة حسب القرب</h2>{locLoading&&<span className="text-xs text-zinc-500">جاري تحديد موقعك...</span>}{locError&&<span className="text-xs text-red-500">{locError}</span>}</div>
          <section className="grid grid-cols-2 md:grid-cols-4 gap-3">{filteredOffers.map(o=><article key={o.id} className="rounded-2xl border bg-white border-[#EADFC9] overflow-hidden flex flex-col"><div className="relative"><img src={o.image} className="h-36 w-full object-cover" alt={o.title}/><span className="absolute top-2.5 right-2.5 px-2.5 py-1 rounded-full text-xs font-black text-white" style={{background:GOLD_DARK}}>-{o.discount}%</span>{o.distance!==null&&<span className="absolute top-2.5 left-2.5 px-2 py-1 rounded-full bg-black/80 text-white text-xs font-bold">📍 {formatDistance(o.distance)}</span>}{o.distance!==null&&o.distance<3&&<span className="absolute bottom-2 right-2 px-2 py-1 rounded-full bg-[#FFF3CC] text-[#7A5A16] text-xs font-black">🔥 قريب منك</span>}</div><div className="p-3 flex flex-col flex-1"><div className="text-xs text-zinc-500">🏬 {o.store} • {o.location.district}</div><h3 className="font-bold text-sm mt-1 min-h-10 line-clamp-2">{o.title}</h3><div className="flex gap-1.5 mt-auto pt-3"><button onClick={()=>openMaps(o.location)} className="flex-1 h-9 rounded-full text-xs font-black text-white" style={{background:GOLD_DARK}}>الوصول 🚗</button><button onClick={()=>setShowMapFor(o)} className="w-9 h-9 rounded-full border bg-[#FFF8E6] border-[#EADFC9]">🗺️</button></div></div></article>)}</section>
        </>}

        {tab==='stores'&&<section className="mt-6"><h2 className="font-black text-base mb-4">المتاجر</h2><div className="grid grid-cols-2 gap-3">{STORES.map(s=><div key={s.id} className="p-4 rounded-2xl bg-white border border-[#EADFC9] flex items-center gap-3"><div className="w-12 h-12 rounded-xl grid place-items-center text-xl" style={{background:`${s.color}15`}}>{s.icon}</div><div className="flex-1"><div className="font-bold text-sm">{s.name}</div><div className="text-xs text-zinc-500">{s.count} عروض • {currentLabel||'جدة'}</div></div><button onClick={()=>{setTab('home'); setNearbyOnly(false)}} className="px-3 h-8 rounded-full bg-[#1F1B16] text-white text-xs font-bold">عرض</button></div>)}</div></section>}

        {tab==='haraj'&&<section className="mt-6"><div className="flex justify-between items-end mb-4"><h2 className="font-black text-base">حراج - مستعمل وجديد</h2><button onClick={()=>showToast('قريبا')} className="px-4 h-9 rounded-full text-xs font-bold text-white" style={{background:GOLD}}>+ أضف إعلان</button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-3">{HARAJ.map(h=><div key={h.id} className="rounded-2xl bg-white border border-[#EADFC9] overflow-hidden flex"><img src={h.image} className="w-28 h-28 object-cover" alt=""/><div className="p-3 flex-1"><div className="font-bold text-sm line-clamp-1">{h.title}</div><div className="text-sm font-black mt-1" style={{color:GOLD_DARK}}>{formatPrice(h.price)}</div><div className="text-xs text-zinc-500 mt-1">{h.district} • {h.time} • {h.seller}</div><div className="flex gap-2 mt-2"><button onClick={()=>showToast('تم فتح المحادثة')} className="flex-1 h-8 rounded-full bg-[#1F1B16] text-white text-xs font-bold">تواصل</button><button onClick={()=>showToast('تمت إضافة للمفضلة')} className="w-8 h-8 rounded-full border border-[#EADFC9]">❤️</button></div></div></div>)}</div></section>}

        {tab==='office'&&<section className="mt-6 space-y-4">
          <div className="rounded-2xl bg-gradient-to-br from-[#1F1B16] to-[#3A2F18] p-5 text-white"><div className="font-black text-base">{user? `أهلاً ${user.name}`:'مكتبي'}</div><div className="text-xs opacity-80 mt-1">نقاطك: 1,240 • وفرت: 860 ر.س هذا الشهر</div><div className="grid grid-cols-3 gap-2 mt-4"><div className="rounded-xl bg-white/10 p-3 text-center"><div className="font-black text-lg">{favs.length}</div><div className="text-xs opacity-80">المفضلة</div></div><div className="rounded-xl bg-white/10 p-3 text-center"><div className="font-black text-lg">{cart.length}</div><div className="text-xs opacity-80">السلة</div></div><div className="rounded-xl bg-white/10 p-3 text-center"><div className="font-black text-lg">3</div><div className="text-xs opacity-80">تنبيهات سعر</div></div></div></div>
          ...
          <div><h3 className="font-bold text-sm mb-2">🔔 تنبيهات الأسعار</h3><div className="rounded-2xl bg-white border border-[#EADFC9] p-4 text-sm">PS5 • ينبهك عند أقل من 1800 ر.س • <span className="text-green-600 font-bold">نشط</span></div></div>
        </section>}

        {tab==='hkeem'&&<section className="mt-6"><div className="rounded-3xl p-6 text-white" style={{background:`linear-gradient(135deg, ${GOLD_DARK}, ${GOLD_LIGHT})`}}><div className="flex items-center gap-3"><div className="w-12 h-12 rounded-xl bg-white grid place-items-center font-black" style={{color:GOLD_DARK}}>ح</div><div><div className="font-black text-lg">متجر حكيم الأصلي</div><div className="text-xs opacity-90">منتجات أصلية • شحن مجاني داخل جدة • دفع عند الاستلام</div></div></div></div><div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">{OFFERS.filter(o=>o.isOwn).map(o=><div key={o.id} className="rounded-2xl bg-white border-2 border-[#E9C86A] overflow-hidden"><img src={o.image} className="h-36 w-full object-cover" alt=""/><div className="p-3"><div className="font-bold text-sm">{o.title}</div><div className="flex items-baseline gap-1 mt-1"><span className="font-black text-sm" style={{color:GOLD_DARK}}>{formatPrice(o.price)}</span><span className="text-xs line-through text-zinc-400">{formatPrice(o.old_price)}</span></div><button onClick={()=>{setCart(c=>[...c,o.id]); showToast('أضيف للسلة 💛')}} className="w-full mt-2 h-9 rounded-full text-xs font-black text-white" style={{background:GOLD_DARK}}>+ أضف للسلة</button></div></div>)}</div></section>}
      </main>
