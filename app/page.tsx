...
<div className="flex items-center gap-2"><img src="/Hkeeem_logo_en-2.png" alt="Hkeeem" className="w-11 h-11 rounded-xl border border-[#EADFC9] bg-white object-contain p-1 shadow-sm"/><span className="font-black text-lg">عروض<span style={{color:GOLD}}>كم</span></span>{cityManual&&<span className="hidden md:inline-flex px-3 py-1 rounded-full text-xs font-bold bg-[#FFF8E6] border border-[#EADFC9] text-[#7A5A16]">📍 {cityManual.label}</span>}</div>
...
<div><div className="flex justify-between text-xs font-bold mb-1"><span>السعر ≤ {priceMax} ر.س</span></div><input type="range" min={50} max={3000} step={50} value={priceMax} onChange={e=>setPriceMax(parseInt(e.target.value))} className="w-full accent-[#B68A2E]"/></div>
